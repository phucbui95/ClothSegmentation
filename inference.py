...
def get_opt():
    ...
    parser.add_argument("--datadir", default = "./datasets/examples")
    parser.add_argument("--datalist", default = "./datasets/examples/list/val.txt")
    parser.add_argument("--outputdir", default = "./output/parsing/val")
    parser.add_argument("--checkpoint", default = "./checkpoint/JPPNet-s2")
    parser.add_argument("--visualization", action='store_true', help='output vis files')
    opt = parser.parse_args()
    return opt

def convert_datalist(datalist):
    paths = []
...
def main(opt):
    ...
    NUM_STEPS = convert_datalist(opt.datalist)
    ...
    with tf.name_scope("create_inputs"):
        reader = ImageReader(opt.datadir, opt.datalist, None, False, False, coord)
        image = reader.image
    ...
    if opt.checkpoint is not None:
        if load(loader, sess, opt.checkpoint):
            print(" [*] Load SUCCESS")
        else:
            print(" [!] Load failed...")
            exit(-1)
    ...
    for step in range(NUM_STEPS):
        try:
            ...
            if opt.visualization:
                parsing_im.save('{}/{}_vis.png'.format(opt.outputdir, img_id))
            cv2.imwrite('{}/{}.png'.format(opt.outputdir, img_id), parsing_[0,:,:,0])
        except:
            break
